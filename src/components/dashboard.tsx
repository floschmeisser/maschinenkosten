    ...
    <main className="page">
      ...
      <section className="stats-grid" aria-label="Uebersicht">
        ...
        <StatCard
          ...
          helper="Beispiel Traktor"
        />
      </section>

      <section className="panel">
        <div className="panel-heading">
          <h2>Dringende Wartung</h2>
          <Link className="button" href={`/${locale}/maintenance`}>
            Wartung oeffnen
          </Link>
        </div>
        <p className="muted">Die wichtigsten Wartungen fuer den naechsten Blick in der Werkstatt.</p>
        <div className="mini-list">
          {todaysWorkTasks.slice(0, 3).map((task) => {
            const machine = machines.find((item) => item.id === task.machineId);
            ...
            return (
              <div className="mini-list-item" key={task.id}>
                <span>
                  ...
                  {getMaintenanceRecurrenceLabel(task)}
                </span>
          ...
          {todaysWorkTasks.length === 0 ? (
            <div className="mini-list-item">
              <span>Keine offenen Wartungen</span>
              <strong>OK</strong>
            </div>
          ) : null}
        </div>
